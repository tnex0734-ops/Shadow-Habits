...
import { Check, LogOut, ScrollText, Zap } from "lucide-react";
...
const characters = [
  ...
  },
  {
    ...
    tagline: "Six Eyes",
    jjkRole: "Tokyo Jujutsu High — Special Grade",
    description: "Limitless confidence, impossible speed, and overwhelming presence.",
    color: "#22D3EE",
    glow: "rgba(34,211,238,0.42)",
    image: "/src/assets/character-gojo.png",
    abilities: ["Limitless", "Infinity", "Hollow Purple"],
    bindingVow: "I will see every possibility and stay unshaken.",
  },
];
    ...
    <div className="h-full overflow-y-auto no-scrollbar px-6 pt-5 pb-6">
      <div className="space-y-5 max-w-2xl mx-auto">
        <div>
          ...
          <p className="text-sm mt-0.5" style={{ color: "rgba(255,255,255,0.4)" }}>Select the sorcerer who will push you to your limits.</p>
        </div>

        <div className="rounded-2xl border p-5 relative overflow-hidden" style={{ background: "rgba(6,12,24,0.75)", borderColor: `${charColor}20`, backdropFilter: "blur(20px)" }}>
          ...
          <div className="relative z-10 flex items-center gap-4">
            <div className="w-16 h-16 rounded-2xl overflow-hidden border-2 flex-shrink-0" style={{ borderColor: charColor, boxShadow: `0 0 20px ${charGlow}` }}>
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1.5">
                <span className="text-xs px-2.5 py-1 rounded-lg font-bold uppercase tracking-wider" style={{ backgroundColor: `${charColor}18`, color: charColor, border: `1px solid ${charColor}30` }}>{currentChar.name}</span>
        ...
        <div>
          <h2 className="font-display text-xs tracking-widest uppercase mb-3" style={{ color: `${charColor}70` }}>All Sorcerers · {characters.length} available</h2>
              ...
              return (
                ...
                >
                  <div className="p-4 flex items-start gap-4">
                    <div className="w-14 h-14 rounded-xl overflow-hidden border-2 flex-shrink-0" style={{ borderColor: isSelected ? char.color : "rgba(255,255,255,0.08)", boxShadow: isSelected ? `0 0 16px ${char.glow}` : "none" }}>
                      <img src={char.image} alt={char.name} className="w-full h-full object-cover" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-0.5">
                        <p className="font-bold text-sm" style={{ color: isSelected ? char.color : "rgba(255,255,255,0.85)" }}>{char.name}</p>
                        <span className="text-[10px] px-1.5 py-0.5 rounded font-semibold uppercase tracking-wider" style={{ background: `${char.color}15`, color: `${char.color}90` }}>{char.tagline}</span>
                      </div>
                      <p className="text-[10px] mb-1.5" style={{ color: "rgba(255,255,255,0.3)" }}>{char.jjkRole}</p>
                          ...
                          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
                            <div className="mt-3 flex flex-wrap gap-1.5">
                              {char.abilities.map((a) => (
                                <span key={a} className="text-[10px] px-2 py-0.5 rounded-full font-semibold" style={{ background: `${char.color}18`, color: char.color, border: `1px solid ${char.color}25` }}>{a}</span>
                              ))}
                            </div>
                          </motion.div>
                        )}
                    ...
                    <div className="flex-shrink-0 ml-2">
                      {isPending ? (
                        ...
                        <div className="w-5 h-5 rounded-full flex items-center justify-center" style={{ background: char.color, boxShadow: `0 0 10px ${char.glow}` }}>
        ...
        <div className="pt-2 border-t" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
          <button onClick={logout} className="flex items-center gap-2 text-sm font-medium px-4 py-3 rounded-xl w-full transition-all" style={{ color: "rgba(255,255,255,0.35)" }} onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.color = "#ff6b6b"; (e.currentTarget as HTMLElement).style.backgroundColor = "rgba(255,50,50,0.06)"; }} onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.color = "rgba(255,255,255,0.35)"; (e.currentTarget as HTMLElement).style.backgroundColor = ""; }}>
